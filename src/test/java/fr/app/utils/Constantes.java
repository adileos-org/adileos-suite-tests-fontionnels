...
public class Constantes {
    ...
    public static String webdriver_ie_driver; // emplacement sur le disque du driver Internet Explorer
                                              // (IEDriverServer.exe)
    public static String webdriver_firefox_bin; // emplacement sur le disque du binaire firefox (firefox.exe)
    ...
    public static void initialiseEnv() {
        ...
        if (loader != null) {
            ...
            try {
                if (in == null) {
                    System.err.println("Pas de lecture du fichier " + propertiesFileName
                            + " (pour JUnit dans Eclipse), in == null");
                    propertiesFileName = "global.properties";
                    in = loader.getResourceAsStream(propertiesFileName);
                    if (in == null) {
                        System.err.println(
                                "Pas de lecture du fichier " + propertiesFileName + " (pour Maven), in == null");
                    }
                }
        ...
        webdriver_firefox_bin = globalProp.getProperty("webdriver.firefox.bin");
        if (webdriver_firefox_bin == null) {
            System.err.println(
                    "webdriver.firefox.bin PAS Renseigne ! => le chemin de firefox.exe doit etre dans le PATH");
        } else {
            System.out.println("webdriver.firefox.bin = " + webdriver_firefox_bin);
        }

    }

    static void displayAllSystemProperties() {
