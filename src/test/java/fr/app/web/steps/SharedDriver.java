 ...
 * scenarios a lot faster since opening and closing a browser for each scenario is pretty slow. To prevent browser state
 * from leaking between scenarios, cookies are automatically deleted before every scenario.
 * </p>
 * <p>
 * A new instance of SharedDriver is created for each Scenario and passed to yor Stepdef classes via Dependency
 * Injection
 * </p>
 * <p>
 * As a bonus, screenshots are embedded into the report for each scenario. (This only works if you're also using the
 * HTML formatter).
 * </p>
 * <p>
 * A new instance of the SharedDriver is created for each Scenario and then passed to the Step Definition classes'
 * constructor. They all receive a reference to the same instance. However, the REAL_DRIVER is the same instance
...
public class SharedDriver extends EventFiringWebDriver {
    ...
    /**
     * Avant de lancer le scenario, on efface tous les cookies de facon a etre proche d'un nouvel utilisateur cet
     * effacement permet de garder le navigateur ouvert pour plusieurs scenarios ce qui accelere beaucoup les tests
     *
     * @param scenario
     *            le scenario qui va etre joué
     */
    @Before
    ...
    /**
     * A la fin du scenario, on demande l'image de l'ecran au navigateur on peut egalement ajouter la video
     *
     * @param scenario
     *            le scenerio qui vient de terminer
     */
    @After
